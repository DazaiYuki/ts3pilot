...
spl_autoload_register(
	static function ( string $class ): void {
		if ( ! str_starts_with( $class, 'Ts3Pilot\\' ) ) {
			return;
		}
		$path = __DIR__ . '/../src/' . str_replace( '\\', '/', substr( $class, strlen( 'Ts3Pilot\\' ) ) ) . '.php';
		if ( is_file( $path ) ) {
			require_once $path;
		}
	}
...
$GLOBALS['__ts3pilot_roles']        = array();
...
function get_option( string $option, $default = false ) {
	return $GLOBALS['__ts3pilot_options'][ $option ] ?? $default;
}
...
function update_option( string $option, $value ): bool {
	$GLOBALS['__ts3pilot_options'][ $option ] = $value;
	return true;
}

function delete_option( string $option ): bool {
	unset( $GLOBALS['__ts3pilot_options'][ $option ] );
	return true;
}
...
function set_transient( string $key, $value, int $expiration ): bool {
	$GLOBALS['__ts3pilot_transients'][ $key ] = array( 'value' => $value, 'expiration' => $expiration );
	return true;
}
...
function get_transient( string $key ) {
	return $GLOBALS['__ts3pilot_transients'][ $key ]['value'] ?? false;
}
...
function get_user_meta( int $user_id, string $key, bool $single = false ) {
	$value = $GLOBALS['__ts3pilot_usermeta'][ $user_id ][ $key ] ?? '';
...
function update_user_meta( int $user_id, string $key, $value ): bool {
	$GLOBALS['__ts3pilot_usermeta'][ $user_id ][ $key ] = $value;
	return true;
}

function current_user_can( string ...$capabilities ): bool {
	return $GLOBALS['__ts3pilot_current_user_can'] ?? false;
}

function get_current_user_id(): int {
	return $GLOBALS['__ts3pilot_current_user'];
}

function is_user_logged_in(): bool {
	return $GLOBALS['__ts3pilot_current_user'] > 0;
}
...
function wp_remote_request( string $url, array $args = array() ) {
	$GLOBALS['__ts3pilot_http_calls'][] = array( 'url' => $url, 'args' => $args );
	$queued = array_shift( $GLOBALS['__ts3pilot_http_queue'] );
	if ( null === $queued ) {
...
function get_role( string $role ) {
	return $GLOBALS['__ts3pilot_roles'][ $role ] ?? null;
}
...
function get_users( array $args = array() ): array {
	return $GLOBALS['__ts3pilot_users'] ?? array();
}
...
function get_current_screen(): ?object {
	return $GLOBALS['__ts3pilot_current_screen'] ?? null;
}
...
function get_userdata( int $user_id ) {
	return $GLOBALS['__ts3pilot_userdata'][ $user_id ] ?? false;
}
