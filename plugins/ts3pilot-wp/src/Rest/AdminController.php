...
final class AdminController {
	...
	public function identity_challenge( WP_REST_Request $request ): WP_REST_Response {
		...
		$code = \Ts3Pilot\Identity\Challenge::start( $user_id );
		...
		try {
			$client->register_identity_challenge(
				array(
					...
					'webhookUrl'    => rest_url( 'ts3pilot/v1/identity/callback' ),
	...
	public function identity_callback( WP_REST_Request $request ): WP_REST_Response {
		$headers  = array(
			'x-ts3pilot-timestamp' => $request->get_header( 'x-ts3pilot-timestamp' ),
			'x-ts3pilot-nonce'     => $request->get_header( 'x-ts3pilot-nonce' ),
			'x-ts3pilot-signature' => $request->get_header( 'x-ts3pilot-signature' ),
		);
