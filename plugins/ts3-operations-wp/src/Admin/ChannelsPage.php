...
final class ChannelsPage {
	...
	public function render(): void {
		...
		echo '<ul class="ts3ops-tree">' . $tree . '</ul>';
		echo '</div>';
	}
	...
	private function render_create_form( array $channels ): void {
		...
		echo '<input type="hidden" name="action" value="ts3cops_channel_create" />';
		wp_nonce_field( 'ts3cops_channel_create', 'ts3cops_nonce' );
	...
	private function render_tree( array $by_parent, int $parent_id, array $all ): string {
		...
		foreach ( $by_parent[ $parent_id ] ?? array() as $channel ) {
			...
			$html    .= '<li>';
			$html    .= '<div class="ts3ops-channel-row"><strong>' . esc_html( $name ) . '</strong>'
				. '<span class="ts3ops-channel-meta">id=' . esc_html( (string) $id ) . ' clients=' . esc_html( (string) $clients ) . '</span></div>';
	...
	private function edit_form( int $channel_id, array $all ): string {
		...
		$html .= '<input type="hidden" name="action" value="ts3cops_channel_edit" />';
		$html .= '<input type="hidden" name="channel_id" value="' . esc_attr( (string) $channel_id ) . '" />';
		wp_nonce_field( 'ts3cops_channel_edit_' . $channel_id, 'ts3cops_nonce' );
	...
	private function move_form( int $channel_id, array $all ): string {
		$html  = '<details class="ts3ops-inline-form"><summary>Move</summary>';
		$html .= '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '">';
		$html .= '<input type="hidden" name="action" value="ts3cops_channel_move" />';
		$html .= '<input type="hidden" name="channel_id" value="' . esc_attr( (string) $channel_id ) . '" />';
		wp_nonce_field( 'ts3cops_channel_move_' . $channel_id, 'ts3cops_nonce' );
	...
	private function delete_form( int $channel_id ): string {
		...
		$html .= '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" data-confirm data-confirm-msg="'
			. esc_attr__( '确定删除该频道？此操作不可撤销。', 'ts3-operations' ) . '">';
		$html .= '<input type="hidden" name="action" value="ts3cops_channel_delete" />';
		$html .= '<input type="hidden" name="channel_id" value="' . esc_attr( (string) $channel_id ) . '" />';
		wp_nonce_field( 'ts3cops_channel_delete_' . $channel_id, 'ts3cops_nonce' );
