...
final class UsersPage {
	public function render(): void {
		...
		foreach ( $users as $user ) {
			...
			echo '<td><span class="ts3ops-status ts3ops-status-' . esc_attr( (string) ( $mapping['status'] ?? 'unbound' ) ) . '">'
	...
	private function render_notice(): void {
		if ( isset( $_GET['ts3cops_code'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$code = sanitize_text_field( wp_unslash( $_GET['ts3cops_code'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			echo '<div class="notice notice-success"><p>'
				. esc_html__( '挑战码（仅显示一次，10 分钟有效）：', 'ts3-operations' )
	...
	private function challenge_form( int $user_id ): string {
		...
		$html .= '<input type="hidden" name="action" value="ts3cops_identity_challenge" />';
		$html .= '<input type="hidden" name="user_id" value="' . esc_attr( (string) $user_id ) . '" />';
		wp_nonce_field( 'ts3cops_identity_challenge_' . $user_id, 'ts3cops_nonce' );
	...
	private function status_form( int $user_id ): string {
		$html  = '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" style="display:inline">';
		$html .= '<input type="hidden" name="action" value="ts3cops_identity_status" />';
		$html .= '<input type="hidden" name="user_id" value="' . esc_attr( (string) $user_id ) . '" />';
		wp_nonce_field( 'ts3cops_identity_status_' . $user_id, 'ts3cops_nonce' );
		$html .= '<input type="text" name="ts3_uid" placeholder="' . esc_attr__( 'TS3 UID (for verified)', 'ts3-operations' ) . '" maxlength="128" />';
